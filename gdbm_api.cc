...
bool Tgdbm_ro::open(const std::string &fName)
{
   ...
   if (!gdb_fd)
   {
      if (gdbm_errno == GDBM_FILE_OPEN_ERROR || gdbm_errno == GDBM_FILE_WRITE_ERROR
...
{
   datum k, v;
   k.dptr=const_cast<char *>(key.c_str()); k.dsize=static_cast<int>(key.size());
   v.dptr=const_cast<char *>(value.c_str()); v.dsize=static_cast<int>(value.size());
